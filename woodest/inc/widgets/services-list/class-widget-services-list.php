...
add_action( 'widgets_init', 'awardthemes_services_list_widget' );
if( !function_exists('awardthemes_services_list_widget') ){
	function awardthemes_services_list_widget() {
		register_widget( 'Widget_Services_List' );
	}
}	
	
class Widget_Services_List extends WP_Widget {
	...
	public function __construct() {
		...
		parent::__construct( false, esc_attr__( 'AwardThemes: Services List', 'woodest' ), $widget_ops );
	}
	...
	public function widget( $args, $instance ) {
		...
		$query_args = array('post_type' => 'services', 'suppress_filters' => false);
		$query_args['posts_per_page'] = $num_fetch;
		$query_args['orderby'] = 'comment_count';
		$query_args['order'] = 'desc';
		$query_args['paged'] = 1;
		$query_args['services_category'] = $category;
		$query_args['ignore_sticky_posts'] = 1;
		$query_args['post__not_in'] = array(get_the_ID());
		$query = new WP_Query( $query_args );
		
		require_once (get_template_directory() .'/inc/widgets/services-list/views/widget.php');
	}
	...
	public function form( $instance ) {
		...
		<p>
			...
			<select class="widefat" name="<?php echo esc_attr($this->get_field_name('category')); ?>" id="<?php echo esc_attr($this->get_field_id('category')); ?>">
				...
				<?php 	
				$category_list = get_terms('services_category');
